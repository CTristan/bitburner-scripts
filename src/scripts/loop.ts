...
export async function main(ns: NS): Promise<void> {
    ...
    await ns.write(hostFile, host, "w")
    ...
    if (ns.getServerMoneyAvailable(host) > 0) {
        for (;;) {
            if (isFactionHacking(ns)) {
                await ns.write(hostFile, "share", "w")
            } else {
                await ns.write(hostFile, host, "w")
            }
            ...
            // Removed Grow to keep script under 4GB. Re-add if able to in the future.
            // await ns.grow(serverToHack)

            await ns.weaken(host)
        }
    } else {
        let serverToHack = await findBestServer(ns)

        for (;;) {
            if (isFactionHacking(ns)) {
                await ns.write(hostFile, "share", "w")
            } else {
                serverToHack = await findBestServer(ns)
                const oldServerToHack = await ns.read(hostFile)
                if (serverToHack !== oldServerToHack) {
                    await ns.write(hostFile, serverToHack, "w")
                }
            }
...
function isFactionHacking(ns: NS): boolean {
    const workDescription = ns.getPlayer().currentWorkFactionDescription

    return workDescription === "carrying out hacking contracts"
}
