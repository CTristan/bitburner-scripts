...
export async function main(ns: NS): Promise<void> {
    ...
    // Check for special API access
    // const player = ns.getPlayer();
    // const hasTixApiAccess = player.hasTixApiAccess;
    ...
    // const tixIncomeFile = 'income-tix.txt'
    // const tixExpenseFile = 'expenses-tix.txt'
    if (firstRun) {
    ...
    while (true) {
        ...
        if (!serverProfits || availableFunds < serverProfits) {
            serverProfits = availableFunds
        }

        if (!hacknetProfits || availableFunds < hacknetProfits) {
 ...
 * @param {NS} ns
 * @param {number} availableFunds How much we have available to spend.
